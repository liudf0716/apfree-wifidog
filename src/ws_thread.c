...
process_ws_msg(struct bufferevent *bev, const char *msg)
{
	...
	// Upward reports (connect/bootstrap) are generated by device on first connect.
	// Invoke gateway-state sync logic for them so behavior matches legacy handling.
	if (strcmp(op_str, "connect") == 0 || strcmp(op_str, "bootstrap") == 0) {
		debug(LOG_INFO, "Processing initial upward report (ws): %s", op_str);
		handle_gateway_state_heartbeat_request(jobj, transport);
		destroy_transport_context(transport);
		json_object_put(jobj);
		return;
	}

	// Heartbeat frames are periodic device reports; ignore here (they are handled by device-side heartbeat sender)
	if (strcmp(op_str, "heartbeat") == 0) {
		debug(LOG_DEBUG, "Ignoring periodic heartbeat message in inbound WS path");
		destroy_transport_context(transport);
		json_object_put(jobj);
		return;
	}
...
{
	json_object *root = build_gateway_report_message(type);
	if (!root) {
		debug(LOG_ERR, "Failed to build %s message", type);
		return;
	}
