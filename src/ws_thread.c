...
process_ws_msg(struct bufferevent *bev, const char *msg)
{
	...
	if (req_id && (json_object_is_type(req_id, json_type_string) ||
		...
		api_transport_context_t *transport = create_websocket_transport_context(bev, NULL);
		if (transport) {
			send_ws_error_response(transport, 400, "Missing or invalid req_id", NULL);
			destroy_transport_context(transport);
		}
		return;
	}
	...
	if (json_object_object_get_ex(jobj, "response", &j_response)) {
	...
	if (!op) {
		debug(LOG_ERR, "Missing 'op' field in WebSocket message");
