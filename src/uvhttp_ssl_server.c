    ...
    )
{
    ...
    ssl_set_rng( &ssl->ssl, ctr_drbg_random, &ssl->ssl.ctr_drbg );
	
	ssl_set_ca_chain( &ssl->ssl, ssl->ssl->srvcert.next, NULL, NULL );
    if( ( ret = ssl_set_own_cert( &ssl->ssl, &ssl->ssl.srvcert, &ssl->ssl.key ) ) != 0 ) {
		goto cleanup;
	}
