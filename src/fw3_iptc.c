...
pthread_mutex_t fw3_iptables_mutex = PTHREAD_MUTEX_INITIALIZER;
...
static void
fw3_rule_print4(struct ipt_entry *e)
{
...
static void
fw3_rule_print(struct fw3_ipt_rule *r, const char *prefix, const char *chain)
{
	printf("iptables -t %s %s %s",fw3_flag_names[r->h->table], prefix, chain);
