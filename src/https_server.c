...
void thread_https_server(void *args) {
    ...
    if ( loop) {
        ...
        if ( server_ssl) {
            ...
            if ( uvhttp_server_ip4_listen( server_ssl, config->gw_address, config->https_server->ssl_port) == UVHTTP_OK) {
                debug(LOG_INFO, "https on %d success", config->https_server->ssl_port);
            }
        }
        uvhttp_run( loop);
