...
if ip link show tap0 > /dev/null 2>&1; then
    ...
    echo "Setting IP address of 'tap0' to 192.168.1.2 in 'aw' namespace"
    sudo ip netns exec aw ip addr add 192.168.1.2/24 dev tap0
    ...
    sudo ip netns exec aw ip route add 0.0.0.0/0 via 192.168.1.1
...
ip route add 100.100.100.0/24 via 172.19.100.80