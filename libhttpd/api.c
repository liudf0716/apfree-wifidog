...
int httpdAddVariable(request *r, const char *name, const char *value)
{
	...
	if(newVar == NULL) {
		fprintf(stderr, "httpdAddVariable malloc newVar failed\n");
		exit(0);
	}
	...
	int	port;
{
	...
	*/
	...
	if(new->content == NULL) {
		fprintf(stderr, "httpdCreate malloc new->content failed\n");
		exit(0);
	}	
	bzero(new->content,sizeof(httpDir));
	new->content->name = strdup("");
	
	pthread_mutex_init(&(new->httpdMutex), NULL);
	...
	struct	timeval *timeout;
{
	...
	if (ipaddr) {	
    	...
    	*/
    memset(r->clientAddr, 0, HTTP_IP_ADDR_LEN);
    inet_ntop(AF_INET, &(addr.sin_addr), r->clientAddr, HTTP_IP_ADDR_LEN);
	r->readBufRemain = 0;
	r->readBufPtr = NULL;
...
int httpdReadRequest(httpd *server, request *r)
{
	...
	*/
	count = 0;
	inHeaders = 1;//0 : not parse header
	while(debug && _httpd_readLine(r, buf, HTTP_MAX_LEN) > 0) 
	{
		count++;
	...
	char	*path;
{
	...
	else
	{
		...
		if(newEntry->path == NULL) {
			fprintf(stderr, "httpdAddFileContent malloc newEntry->path failed\n");
			exit(0);
		}
	...
	char	*path;
{
	...
	else
	{
		...
		newEntry->path = malloc(strlen(server->fileBasePath) +
			strlen(path) + 2);
		if(newEntry->path == NULL) {
			fprintf(stderr, "httpdAddWildcardContent newEntry->path malloc failed\n");
			exit(0);
		}
		snprintf(newEntry->path, HTTP_MAX_URL, "%s/%s",
			server->fileBasePath, path);
...
httpDir* httpdProcessRequestDir(httpd *server, request *r, char *dirName) {
	httpDir	*dir;
...
void httpdProcessRequestEntry(httpd *server, request *r, httpDir *dir, char *entryName) {	
	httpContent *entry;
