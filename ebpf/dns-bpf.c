...
static __always_inline int dns_monitor_main(struct __sk_buff *skb)
{
    ...
    if ((void *)(eth + 1) > data_end) {
        bpf_printk("[DNS-BPF] drop: eth header out of bounds");
        return TC_ACT_OK;
    }
    ...
    if (h_proto == ETH_P_IP) {
        ...
        if ((void *)(ip + 1) > data_end) {
            bpf_printk("[DNS-BPF] drop: ip header out of bounds");
            return TC_ACT_OK;
        }
    ...
    // 处理IPv6数据包
        ...
        if ((void *)(ipv6 + 1) > data_end) {
            bpf_printk("[DNS-BPF] drop: ipv6 header out of bounds");
            return TC_ACT_OK;
        }
        ...
        if (ip_proto != IPPROTO_UDP) {
            bpf_printk("[DNS-BPF] drop: ipv6 not UDP");
            return TC_ACT_OK;
        }
    } else {
        bpf_printk("[DNS-BPF] drop: not IPv4/IPv6");
    ...
    if ((void *)(udp + 1) > data_end) {
        bpf_printk("[DNS-BPF] drop: udp header out of bounds");
        return TC_ACT_OK;
    }
    ...
    if ((void *)(dns_payload_start + sizeof(struct dns_hdr)) > data_end) {
        bpf_printk("[DNS-BPF] drop: dns header out of bounds");
        return TC_ACT_OK;
    }
    ...
    bpf_printk("[DNS-BPF] DNS packet: src=%d dst=%d QR=%d", src_port, dst_port, is_response);
    ...
    // 只监控DNS响应，过滤掉查询
    if (!is_response) {
        bpf_printk("[DNS-BPF] drop: not DNS response");
        return TC_ACT_OK;
    }
    ...
    if (!output_data) {
        bpf_printk("[DNS-BPF] drop: ringbuf reserve failed");
    ...
    bpf_printk("[DNS-BPF] ringbuf filled: src=%d dst=%d len=%d", src_port, dst_port, dns_payload_len);
